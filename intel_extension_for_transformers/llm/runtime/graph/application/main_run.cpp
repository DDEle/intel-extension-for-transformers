  ...
  while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
    ...
    if (embd.size() > 0) {
      ...
      if (n_past + (int)embd.size() > n_ctx) {
        ...
        if (!params.non_roped_k) {  // non_roped_k can use ring-buffer and thus does not need re-computing
          ...
          NE_ASSERT(("n_discard cannot be used with non_roped_k!", n_discard == -1 || n_discard == 1));
        }
      }
